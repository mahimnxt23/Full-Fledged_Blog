...
gravatar = Gravatar(app, size=50, rating='g', default='retro', force_default=False,
                    ...
                    base_url=None)  # modified size has a default value of '100'...
...
# @admin_only  # notice the "@admin-only decorator"  # commented this line so everyone can make posts...
def create_post():
...
@admin_only  # notice the "@admin-only" decorator...
@login_required
def edit_post(post_id):
    ...
    if edit_post_form.validate_on_submit():
        # short method, so I chose this...
        edit_post_form.populate_obj(selected_post)
        db.session.commit()

        # longer method, kept this for understanding short methods BEHIND THE SCENE concept 👇🏻👆🏻...
